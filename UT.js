...
var A, Base, INFINITE_LOOP_DETECTION_ITERATIONS, O, UT, UTUT, V, aGenerate, bRan, bRunning, bag, decorate, fail, fs, handler, iterations, msStart, pass, path, proxyBag, sectionGenerate, t_depth, target, testGenerate, testIndex, testList, testListSaved, testStack, trace, util;
...
module.exports = UT = class UT extends Base {
  ...
  A(tn, fn) {
    return aGenerate('A').bind(this)(tn, fn);
  }
  ...
  S(tn, fn) {
    return sectionGenerate('S').bind(this)(tn, fn);
  }
  ...
  T(tn, fn) {
    return testGenerate('T').bind(this)(tn, fn);
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (testList.length > 0) {
        ...
        if (bFoundOverride) {
          ...
          this.log(`found ${testListSaved} tests, but also found ${testList.length} overrides`);
        }
        if (testList.length > 0) {
          testListSaved = testList.length;
          this.next();
