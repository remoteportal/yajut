// coffeeScript.coffee: Generated by CoffeeScript 2.3.1 (coffeescript.coffee IMMED5 affects ./coffee -v)
  ...
  //if node
var ANEW, AP, ASS, AsyncTest, BB, Base, C, Context, DATE, EXPORTED, GG, HM, IF, IS, LL, MAKEa, MAKEs, MAKEt, N, O, ONEW, REMOVE_ME_MS, S, Section, SyncTest, TYPE, Test, Type, UT, UTBase, UTRunner, UT_UT, Util, V, VNEW, abort, bag, drill, duck, handler, json, kt, kvt, log, modMap, path, proxyBag, syncTestsCount, target, testList, testStack, textFormat, trace, type, vt,
  indexOf = [].indexOf;
...
EXPORTED = UT = class UT extends UTBase { //@UT
  ...
  a(a, b, c) {
    return MAKEa('a').bind(this)(a, b, c);
  }
  ...
  S(a, b, c) {
    return MAKEs('S').bind(this)(a, b, c);
  }
  ...
  T(a, b, c) {
    return MAKEt('T').bind(this)(a, b, c);
  }
...
//		@log "failList.length=#{@failList.length}"
  ...
  decorate() {
    ...
    this.logSilent = function(s, o, opt) {
      return Context.logBase(this.one(), s, o, {
        bVisible: false //REVISIT
      });
    };
    this.logTransient = function(s, o, opt) { //REVISIT
      if (this.runner.OPTS.mFailMode === this.FM_RUNALL) {
        return Context.logBase(this.one(), `TRANSIENT: ${s}`, o, opt);
      } else {
        Context.logBase(this.one(), `FATAL_TRANSIENT: ${s}`, o, opt);
        return abort("logError called with @mFailMode is @FM_RUNALL=false");
      }
    };
...
UTRunner = class UTRunner extends UTBase { //@UTRunner @runner
  ...
  exit(mWhy1, msg) {
    ...
    this.summary = {
      ...
      frag: this.frag = `[${this.secsElapsed}s]  passHERE=${this.pass} fail555=${this.failList.length}`,
      mWhy: this.mWhy,
      pass: this.pass,
  ...
  testsEnable() {
    ...
    if (testList.length > 0) {
      ...
      return this.log(`${this.summary} Found ${testList.length} ${S.PLURAL("test", testList.length)}${AP.d(this.enabledCnt < testList.length, `with ${this.enabledCnt} enabled`)}`);
    }
  }
...
UT_UT = class UT_UT extends UT { //@UT_UT		@unittest  @ut
  run() {
    this.s("@p - async return implicit Promise", function() {
      ...
      return this.p("role model of how to use @p and @ce together", {
        desc: "see file header for full explanation"
      }, function() {
        var pr;
